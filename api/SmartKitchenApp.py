...
def getInventory():
    ...
    try:
        with open('json/inventory.json', 'r') as json_file:
            inventory = json.load(json_file)  # this will throw correct errors
            return json.dumps(inventory) # Response(json.dumps(inventory), mimetype='application/json')
    except IOError:
    ...
    return jsonify(lista)#json.dumps(lista)
    '''
